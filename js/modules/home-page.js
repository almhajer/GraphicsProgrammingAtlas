...
(function (global) {
  ...
  function showHomePage(options) {
    ...
    content.innerHTML = '<section class="home-hero"><div class="home-hero-main">'
      ...
      + '<p class="home-hero-lead">الصفحة الرئيسية تعرض الآن كل مكتبات المشروع بنفس قالب Vulkan الحالي: بطاقات أقسام، مسار بداية، نقاط دخول سريعة، وروابط مرجعية، مع ظهور فروع SDL3 المتخصصة مثل SDL3Audio وSDL3_image وSDL3_mixer وSDL3_ttf داخل المسار نفسه بدل بقائها مخفية خلف الشريط الجانبي فقط.</p>'
      ...
      + '</div>'
      + '<div class="home-hero-subactions">'
      + '<div class="home-hero-subactions-label">فروع SDL3 السريعة</div>'
      + '<div class="home-hero-actions home-hero-actions-packages">'
      + '<button onclick="showSdl3PackageIndex(\'audio\')" class="quick-btn">' + icon('sdl3', 'ui-codicon list-icon', 'SDL3Audio') + ' افتح SDL3Audio</button>'
      + '<button onclick="showSdl3PackageIndex(\'image\')" class="quick-btn">' + icon('sdl3', 'ui-codicon list-icon', 'SDL3_image') + ' افتح SDL3_image</button>'
      + '<button onclick="showSdl3PackageIndex(\'mixer\')" class="quick-btn">' + icon('sdl3', 'ui-codicon list-icon', 'SDL3_mixer') + ' افتح SDL3_mixer</button>'
      + '<button onclick="showSdl3PackageIndex(\'ttf\')" class="quick-btn">' + icon('sdl3', 'ui-codicon list-icon', 'SDL3_ttf') + ' افتح SDL3_ttf</button>'
      + '</div>'
      + '</div>'
