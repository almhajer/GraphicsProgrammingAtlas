...
function renderIdentifierToken(token) {
  ...
  const ffmpegSearchEntries = runtimeGlobal.__ARABIC_VULKAN_BOOTSTRAP_CONTEXT__?.getFfmpegSearchEntries?.() || [];
  const ffmpegEntry = Array.isArray(ffmpegSearchEntries)
    ? ffmpegSearchEntries.find((item) => {
      const itemName = String(item?.name || '').trim();
      if (itemName === token) {
        return true;
      }
      const aliases = Array.isArray(item?.aliases) ? item.aliases : [];
      return aliases.some((alias) => String(alias || '').trim() === token);
    })
    : null;

  const buildFfmpegFallbackToken = (text, iconType, tooltipText) => {
    const tooltip = String(tooltipText || text).trim();
    const aria = escapeAttribute(`${text}: ${tooltip.replace(/\n/g, ' - ')}`);
    return `<span class="code-token entity-link-with-icon" data-tooltip="${escapeAttribute(tooltip)}" tabindex="0" aria-label="${aria}">${safeRenderEntityLabel(text, iconType, {code: true})}</span>`;
  };
  ...
  if (ffmpegEntry?.kind && ffmpegEntry?.slug) {
    const tooltip = String(ffmpegEntry.titleArabic || ffmpegEntry.description || token).trim();
    const iconType = ffmpegEntry.iconType || (
      ffmpegEntry.kind === 'functions'
        ? 'function'
        : (ffmpegEntry.kind === 'structures'
          ? 'structure'
          : (ffmpegEntry.kind === 'enums'
            ? 'enum'
            : (ffmpegEntry.kind === 'constants' ? 'constant' : 'file')))
    );
    const className = 'code-token code-link entity-link-with-icon';
    return `<a href="#ref/ffmpeg/${escapeAttribute(ffmpegEntry.kind)}/${escapeAttribute(ffmpegEntry.slug)}" class="${className}" title="${escapeAttribute(token)}" data-tooltip="${escapeAttribute(tooltip)}" tabindex="0" aria-label="${escapeAttribute(`${token}: ${tooltip.replace(/\n/g, ' - ')}`)}" onclick="showReferenceEntity('ffmpeg', '${escapeAttribute(ffmpegEntry.kind)}', '${escapeAttribute(ffmpegEntry.slug)}'); return false;">${safeRenderEntityLabel(token, iconType, {code: true})}</a>`;
  }

  if (/^(?:av|sws|swr)_[a-z0-9_]+$/.test(token)) {
    return buildFfmpegFallbackToken(
      token,
      'function',
      'دالة من عائلة FFmpeg تظهر داخل الأمثلة الحالية، لكن لا توجد لها بعد صفحة محلية مستقلة في هذا المرجع. يوضح الرمز هنا أنها استدعاء تنفيذي ضمن مسار الوسائط.'
    );
  }

  if (/^AVMEDIA_TYPE_[A-Z0-9_]+$/.test(token) || /^AV_[A-Z0-9_]+$/.test(token)) {
    return buildFfmpegFallbackToken(
      token,
      'constant',
      'ثابت أو قيمة تعداد من FFmpeg. يظهر هذا الرمز في المثال لتحديد نوع وسائط أو علم أو حالة تشغيلية داخل المسار.'
    );
  }

  if (/^AV[A-Z][A-Za-z0-9_]*$/.test(token)) {
    return buildFfmpegFallbackToken(
      token,
      'structure',
      'نوع أو بنية من FFmpeg يحمل حالة تشغيلية أو بيانات وسيطة داخل المثال، مثل السياقات والإطارات والحزم ومخططات الفلاتر.'
    );
  }
...
function highlightSingleCodeBlock(block, options = {}) {
  ...
  const referenceEntity = referenceLibrary
    ? {
...
function setMobileSidebarOpenState(open) {
  const sidebar = document.getElementById('sidebar');
...
function initMobileSidebarToggle() {
  ...
  setMobileSidebarOpenState(sidebar.classList.contains('open'));

  toggle.addEventListener('click', (event) => {
    if (event.__mobileSidebarHandled || event.defaultPrevented) {
      return;
    }
    if (window.innerWidth > 900) {
      return;
  ...
  document.addEventListener('click', (event) => {
    ...
    setMobileSidebarOpenState(false);
  });
  ...
  sidebar.addEventListener('click', (event) => {
    ...
    const navItem = event.target instanceof Element ? event.target.closest('.nav-item[data-nav-type]') : null;
    if (!navItem) {
      return;
    }
    window.setTimeout(() => setMobileSidebarOpenState(false), 0);
  });
