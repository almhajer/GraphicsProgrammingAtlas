...
const SDL3_PACKAGE_KEYS = ['core', 'image', 'mixer', 'ttf'];
...
function getSdl3PackageKeyFromClusterId(clusterId) {
  const match = /^sdl3-(core|image|mixer|ttf)-cluster$/.exec(String(clusterId || ''));
  return match ? match[1] : '';
}

function getSdl3VisiblePackageKeys() {
  return SDL3_PACKAGE_KEYS.filter((packageKey) => !!getSdl3PackageInfo(packageKey));
}
