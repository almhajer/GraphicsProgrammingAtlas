...
(function (global) {
  ...
  function getSearchIndexSectionKeys(filter = 'all') {
    if (filter && filter !== 'all') {
      if (filter === 'sdl3-audio') {
        return ['sdl3'];
      }
      return [filter];
    }

    return Object.keys(searchFilterLabels).filter((key) => key !== 'all' && key !== 'sdl3-audio');
  }
  ...
  function getFfmpegSearchEntries() {
    return appApi.getFfmpegSearchEntries() || [];
  }
  ...
  function isSearchQueryReady(text) {
    const raw = String(text || '').trim();
    if (!raw) {
      return false;
    }

    if (searchSymbolPattern.test(raw)) {
      return true;
    }
  ...
  function buildGlslSearchIndexSection() {
    const entries = [];
    const glslItems = getAllGlslReferenceItems();
    ...
    entries.push({
      ...
      subsection: 'index',
      name: 'مرجع GLSLang',
      description: 'المدخل المحلي لمكتبة GLSL وGLSLang داخل المشروع.',
      searchText: [
        'GLSL',
        'GLSLang',
        'glsl',
      ...
      item: {
        sectionKey: '',
        sectionTitle: 'GLSLang'
      }
    });

    const seenSections = new Set();
    glslItems.forEach((item) => {
      ...
      entries.push({
        ...
        name: sectionTitle || sectionKey,
        description: `قسم ${sectionTitle || sectionKey} داخل مرجع GLSLang.`,
        searchText: [
          'GLSL',
          'GLSLang',
          'glsl',
  ...
  function buildSdl3SearchIndexSection() {
    ...
    const packageDescriptionMap = {
      core: 'المدخل العام لـ SDL3 الأساسية: النوافذ، الأحداث، الإدخال، والرسم والتكامل مع النظام.',
      audio: 'مرجع SDL3Audio لفتح أجهزة الصوت، إدارة SDL_AudioStream، تحويل PCM، وتنسيقات الصوت الرسمية.',
      image: 'مرجع SDL3_image لتحميل الصور والرسوم المتحركة وتحويلها إلى Surface أو Texture قابلة للاستخدام.',
      mixer: 'مرجع SDL3_mixer لتشغيل ومزج المؤثرات الصوتية والمسارات الصوتية الجاهزة.',
      ttf: 'مرجع SDL3_ttf لتحميل الخطوط وقياس النصوص ورسمها داخل SDL3.'
    };

    getSdl3SearchEntries().forEach((item) => {
      const packageKey = String(item.packageKey || '').trim();
      const packageDisplayName = item.packageDisplayName || item.packageName || packageKey || 'SDL3';
      if (packageKey && !packageEntries.has(packageKey)) {
        packageEntries.set(packageKey, {
          type: 'sdl3-package-index',
          section: 'sdl3',
          subsection: packageKey,
          name: packageDisplayName,
          description: packageDescriptionMap[packageKey] || `المدخل المحلي لحزمة ${packageDisplayName} داخل قسم SDL3.`,
          searchText: [
            packageDisplayName,
            item.packageName,
            packageKey,
            packageDescriptionMap[packageKey],
            packageKey === 'audio' ? 'SDL3Audio SDL3 Audio صوت audio stream pcm wav أجهزة الصوت' : ''
          ].filter(Boolean).join(' '),
          item: {
            packageKey,
            packageDisplayName,
            packageName: item.packageName || packageDisplayName,
            iconType: 'sdl3',
            description: packageDescriptionMap[packageKey] || ''
          }
        });
      }

      const previewText = item.description || getSdl3SearchPreview(item);
      const packageLabel = packageDisplayName;
      entries.push({
        type: 'sdl3',
        section: 'sdl3',
  ...
  function buildCppSearchSectionLookup() {
    ...
    const visitSection = (section, parentMeta = null) => {
      const sectionMeta = {
        key: String(section?.key || parentMeta?.key || '').trim(),
        title: String(section?.title || parentMeta?.title || '').trim(),
        iconType: String(section?.iconType || parentMeta?.iconType || 'cpp').trim() || 'cpp'
      };
      ...
      (section?.tokens || []).forEach((token) => {
        ...
        lookup[normalizedToken] = sectionMeta;
      });

      (section?.subsections || []).forEach((subsection) => visitSection(subsection, sectionMeta));
    };
  ...
  function collectCppHomeTokens() {
    ...
    const visitSection = (section) => {
      ...
      (section?.subsections || []).forEach((subsection) => visitSection(subsection));
    };
  ...
  function buildCppSearchIndexSection() {
    ...
    (Array.isArray(homeConfig?.sections) ? homeConfig.sections : []).forEach((section) => {
      ...
      (section.subsections || []).forEach((sub) => {
        entries.push({
          ...
          section: 'cpp',
          subsection: String(section.key || 'index').trim() || 'index',
          name: String(sub.title || '').trim(),
          description: 'قسم فرعي في ' + (section.title || 'C++'),
          searchText: [
            'C++',
            'cpp',
            section.title,
          ...
          item: {
            sectionKey: String(section.key || '').trim(),
            iconType: String(section.iconType || 'cpp').trim() || 'cpp',
            kindLabel: 'قسم فرعي C++',
            sectionTitle: section.title || 'C++'
          }
        });
        ...
        (sub.subsections || []).forEach((ss) => {
          entries.push({
            ...
            section: 'cpp',
            subsection: String(section.key || 'index').trim() || 'index',
            name: String(ss.title || '').trim(),
            description: 'قسم فرعي في ' + sub.title + ' — ' + (section.title || 'C++'),
            searchText: [
              'C++',
              'cpp',
              section.title,
            ...
            item: {
              ...
              iconType: String(section.iconType || 'cpp').trim() || 'cpp',
              kindLabel: 'قسم فرعي C++',
              sectionTitle: (sub.title || '') + ' — ' + (section.title || 'C++')
            }
          });
        });
      });
    ...
    tokens.forEach((token) => {
      ...
      const displayName = String(merged.title || token).trim();
      entries.push({
        type: 'cpp',
        section: 'cpp',
  ...
  function buildFfmpegSearchIndexSection() {
    const entries = [];
    const ffmpegItems = getFfmpegSearchEntries();
    ...
    entries.push({
      ...
      subsection: 'index',
      name: 'مرجع FFmpeg',
      description: 'مرجع عربي عملي لـ FFmpeg: المكتبات الفرعية والدوال والبنى والتعدادات والثوابت الأساسية.',
      searchText: [
        'FFmpeg',
        'ffmpeg',
        'مرجع FFmpeg',
      ...
      item: {sectionKey: '', kindLabel: 'مرجع FFmpeg', sectionTitle: 'FFmpeg'}
    });

    ffmpegItems.forEach((item) => {
      entries.push({
        type: 'ffmpeg',
        section: 'ffmpeg',
        subsection: item.kind || 'module',
        name: item.name,
        description: item.description || item.shortDescription || item.titleArabic || '',
        searchText: [
          item.name,
          item.displayName,
          item.slug,
  ...
  function buildSearchIndexSection(sectionKey = 'all') {
    switch (sectionKey) {
      ...
      case 'ffmpeg':
        return buildFfmpegSearchIndexSection();
      case 'tutorials':
  ...
  function isSearchEntryVisibleInFilter(entry, filter = 'all') {
    if (filter === 'all') {
      return true;
    }

    if (filter === 'sdl3-audio') {
      const packageKey = String(entry?.item?.packageKey || '').trim();
      return entry?.section === 'sdl3' && (
        packageKey === 'audio' ||
        (entry?.type === 'sdl3-package-index' && entry?.item?.packageKey === 'audio')
      );
    }
  ...
  function getSearchScopeLabel(result) {
    ...
    if (result.type === 'sdl3-package-index') {
      return 'SDL3';
    }
  ...
  function getRankedSearchResults(query, options = {}) {
    ...
    const results = ensureSearchIndex(filter)
      .filter((entry) => isSearchEntryVisibleInFilter(entry, filter))
  ...
  async function ensureSegmentsForActiveFilter() {
    ...
    if (activeSearchFilter === 'sdl3-audio') {
      await ensureUiSegment('sdl3Search');
      return;
    }
    ...
    if (activeSearchFilter === 'cpp') {
      await Promise.all([
        ensureUiSegment('cppHome'),
        ensureUiSegment('cppReferenceData'),
        ensureUiSegment('cppReferenceEnrichment'),
        ensureUiSegment('cppReferenceGuides'),
        ensureUiSegment('cppReferenceOfficialLinks')
      ]);
      invalidateSearchIndex();
      return;
    }

    if (activeSearchFilter === 'ffmpeg') {
  ...
  function getSearchResultIconType(result) {
    ...
    switch (result.type) {
      ...
      case 'ffmpeg':
        return result.item?.iconType || 'file';
      case 'ffmpeg-index':
        return 'file';
  ...
  function displaySearchSuggestions(results, query) {
    ...
    if (!results.length || !isSearchQueryReady(query)) {
      hideSearchSuggestions();
      return;
    }
  ...
  function openSearchEntry(result) {
    ...
    switch (result.type) {
      ...
      case 'glsl-index':
        showGlslIndex();
        if (result.item?.sectionKey) {
          setTimeout(() => scrollToAnchor(`glsl-section-${result.item.sectionKey}`), 30);
        }
        break;
      case 'glsl':
      ...
      case 'sdl3':
        ...
        break;
      case 'sdl3-package-index':
        showSdl3PackageIndex(result.item?.packageKey || 'core');
        break;
      case 'ffmpeg':
        showReferenceEntity('ffmpeg', result.item.kind, result.item.slug);
        break;
      case 'ffmpeg-index':
        if (result.item?.sectionKey) {
          showReferenceKindIndex('ffmpeg', result.item.sectionKey);
        } else {
          showReferenceLibraryIndex('ffmpeg');
        }
        break;
      case 'cmake':
  ...
  async function retrySearchWithFilters(filterKey = 'all', subFilterKey = 'all') {
    ...
    if (isSearchQueryReady(query) && getCurrentView() !== 'search') {
      await handleSearch(query);
    }
  }
  ...
  async function updateSearchSuggestions(query) {
    ...
    if (!isSearchQueryReady(searchQuery)) {
      hideSearchSuggestions();
      return;
    }
  ...
  function displaySearchResults(results) {
    ...
    const labels = {
      ...
      cpp: 'مرجع C++',
      'cpp-index': 'فهرس C++',
      'glsl-index': 'فهرس GLSLang',
      imgui: 'مرجع Dear ImGui',
      glsl: 'GLSLang',
      sdl3: 'مرجع SDL3',
      'sdl3-package-index': 'حزمة SDL3',
      ffmpeg: 'FFmpeg',
      'ffmpeg-index': 'فهرس FFmpeg',
      cmake: 'مرجع CMake',
      tutorial: 'درس',
    ...
    results.forEach((result) => {
      const icons = {
        ...
        'glsl-index': renderEntityIcon('glsl', 'ui-codicon result-icon', 'GLSLang'),
        ...
        'sdl3-package-index': renderEntityIcon('sdl3', 'ui-codicon result-icon', 'SDL3'),
        ffmpeg: renderEntityIcon(result.item?.iconType || 'file', 'ui-codicon result-icon', 'FFmpeg'),
        'ffmpeg-index': renderEntityIcon('file', 'ui-codicon result-icon', 'FFmpeg'),
      ...
      const onclick = {
        ...
        'glsl-index': result.item?.sectionKey
          ? `showGlslIndex(); setTimeout(() => scrollToAnchor('glsl-section-${escapeAttribute(result.item.sectionKey)}'), 30);`
          : 'showGlslIndex()',
        ...
        'sdl3-package-index': `showSdl3PackageIndex('${escapeAttribute(result.item?.packageKey || 'core')}')`,
        ffmpeg: `showReferenceEntity('ffmpeg', '${escapeAttribute(result.item.kind || '')}', '${escapeAttribute(result.item.slug || '')}')`,
        'ffmpeg-index': result.item?.sectionKey
          ? `showReferenceKindIndex('ffmpeg', '${escapeAttribute(result.item.sectionKey)}')`
          : `showReferenceLibraryIndex('ffmpeg')`,
      ...
      html += `
        <div class="search-result-item" onclick="${onclick[result.type]}">
          ...
          <span class="result-type">${result.type === 'sdl3'
            ...
            : result.type === 'sdl3-package-index'
              ? 'حزمة SDL3'
            : result.type === 'imgui'
          ...
          <span class="result-desc">${result.type === 'command'
            ...
            : result.type === 'constant-ref'
              ...
              : result.type === 'sdl3-package-index'
                ? escapeHtml(result.item.description || '')
              : result.type === 'ffmpeg'
                ? escapeHtml(result.item.shortDescription || result.item.titleArabic || '')
              : result.type === 'ffmpeg-index'
                ? escapeHtml(result.description || '')
