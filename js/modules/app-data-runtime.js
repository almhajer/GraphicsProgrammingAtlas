(function (global) {
  ...
  function createAppDataRuntime(options = {}) {
    ...
    const applyGlslExampleTooltipOverrideData = typeof options.applyGlslExampleTooltipOverrideData === 'function'
      ...
      : () => {};
    const applyCppReferenceData = typeof options.applyCppReferenceData === 'function'
      ? options.applyCppReferenceData
      : () => {};
    ...
    const setCmakeSearchEntries = typeof options.setCmakeSearchEntries === 'function'
      ...
      : () => {};
    const setFfmpegSearchEntries = typeof options.setFfmpegSearchEntries === 'function'
      ? options.setFfmpegSearchEntries
      : () => {};
    ...
    async function fetchJsonData(sourcePath) {
      ...
      const resolvedSourcePath = `${sourcePath}${separator}v=${appAssetVersion}`;
      ...
      try {
        ...
        const parseMessage = error instanceof Error ? error.message : 'تعذر تحليل JSON.';
        throw new Error(`فشل تحليل JSON من ${sourcePath}: ${parseMessage}${preview ? ` | بداية المحتوى: ${preview}` : ''}`);
      }
    }
    ...
    async function applyUiSegmentJsonData(segment, payload = {}) {
      ...
      if (segment === 'ffmpegSearch') {
        setFfmpegSearchEntries(Array.isArray(payload.entries) ? payload.entries : []);
        invalidateSearchIndex();
        return;
      }
      ...
      if (segment === 'cppReferenceData') {
        ...
        for (const file of files) {
          ...
          const chunkPayload = await fetchJsonData(sourcePath);
          Object.assign(mergedReferenceData, chunkPayload?.referenceData || {});
        }
        applyCppReferenceData({referenceData: mergedReferenceData});
        return;
      }

      if (segment === 'cppReferenceEnrichment') {
        const enrichFiles = Array.isArray(payload?.files) ? payload.files : [];
        if (enrichFiles.length > 0) {
          ...
          for (const file of enrichFiles) {
            ...
            const chunkPayload = await fetchJsonData(sourcePath);
            Object.assign(mergedEnrichment, chunkPayload?.referenceEnrichment || {});
          }
          applyCppReferenceEnrichmentData({referenceEnrichment: mergedEnrichment});
        } else {
      ...
      if (segment === 'cppReferenceGuides') {
        const guideFiles = Array.isArray(payload?.files) ? payload.files : [];
        if (guideFiles.length > 0) {
          const mergedGuides = {};
          let mergedDeepGuides = {};
          for (const file of guideFiles) {
            const sourcePath = String(file?.path || '').trim();
            if (!sourcePath) { continue; }
            const chunkPayload = await fetchJsonData(sourcePath);
            Object.assign(mergedGuides, chunkPayload?.referenceGuides || {});
            if (chunkPayload?.deepGuides) {
              mergedDeepGuides = {...mergedDeepGuides, ...chunkPayload.deepGuides};
            }
          }
          applyCppReferenceGuideData({referenceGuides: mergedGuides, deepGuides: mergedDeepGuides});
        } else {
