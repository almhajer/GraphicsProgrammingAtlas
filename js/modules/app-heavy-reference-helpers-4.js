...
function highlightSingleCodeBlock(block, options = {}) {
  ...
  void ensureHeavyHelper4RuntimeLoaded()
    .then((loadedRuntime) => loadedRuntime?.highlightSingleCodeBlock?.(block, options))
    .catch(() => {});
}
...
function highlightCodeNow(root = document, options = {}) {
  ...
  void ensureHeavyHelper4RuntimeLoaded()
    .then((loadedRuntime) => loadedRuntime?.highlightCodeNow?.(root, options))
    .catch(() => {});
}
...
function highlightCode(root = document, options = {}) {
  ...
  void ensureHeavyHelper4RuntimeLoaded()
    .then((loadedRuntime) => loadedRuntime?.highlightCode?.(root, options))
    .catch(() => {});
}
