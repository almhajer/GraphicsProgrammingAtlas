window.__ARABIC_VULKAN_SIDEBAR_NAVIGATION__ = (() => {
  ...
  const ffmpegExpandedSections = new Set();
  const ffmpegSidebarIndexCache = new Map();
  ...
  function getFfmpegKindCodicon(kindId = '') {
    switch (String(kindId || '').trim()) {
      case 'functions':
  ...
  async function loadFfmpegSidebarIndex(kindId = '') {
    const normalizedKindId = String(kindId || '').trim();
    if (!normalizedKindId) {
      return null;
    }
    if (!ffmpegSidebarIndexCache.has(normalizedKindId)) {
      ffmpegSidebarIndexCache.set(
        normalizedKindId,
        fetch(`content/reference/ffmpeg/${encodeURIComponent(normalizedKindId)}/index.json`)
          .then((response) => {
            if (!response.ok) {
              throw new Error(`Failed to load FFmpeg sidebar index: ${normalizedKindId}`);
            }
            return response.json();
          })
      );
    }
    return ffmpegSidebarIndexCache.get(normalizedKindId);
  }

  function renderFfmpegSidebarItem(item = {}, kindId = '') {
    const kindCodicon = getFfmpegKindCodicon(kindId);
    const name = String(item.name || '').trim();
    const slug = String(item.slug || '').trim();
    const tooltip = String(item.titleArabic || item.shortDescription || name).trim();
    if (!name || !slug) {
      return '';
    }

    return `<div class="nav-item" data-nav-type="ffmpeg-entity" data-nav-name="${escapeAttribute(name)}" data-nav-library="ffmpeg" data-nav-kind="${escapeAttribute(kindId)}" data-nav-slug="${escapeAttribute(slug)}" data-tooltip="${escapeAttribute(tooltip)}" tabindex="0" role="button" onclick="event.stopPropagation(); handleSidebarNavActivation(event, this); return false;"><span class="ui-codicon nav-icon" data-codicon="${kindCodicon}" aria-hidden="true"></span><span>${escapeHtml(name)}</span></div>`;
  }

  function renderFfmpegSidebarGroups(indexData = {}, kindId = '') {
    const groups = Array.isArray(indexData?.groups) ? indexData.groups : [];
    if (!groups.length) {
      return '';
    }
    ...
    return groups.map((group, groupIndex) => {
      ...
      const groupKey = `${kindId}-${groupIndex}-${String(group.title || group.letter || 'group').trim()}`;
      return `
        <div class="nav-constant-group file-nav-group ffmpeg-subsection-nav-group collapsed" data-ffmpeg-group="${escapeAttribute(groupKey)}">
          <div class="nav-constant-group-header" onclick="toggleConstantGroup(this)">
            <span class="nav-constant-group-title-wrap">
              ...
              <span class="nav-constant-group-title">${escapeHtml(group.title || group.letter || 'مجموعة FFmpeg')}</span>
            </span>
            <span class="nav-constant-group-count">${items.length}</span>
          </div>
          <div class="nav-constant-group-items">
            ${items.map((item) => renderFfmpegSidebarItem(item, kindId)).join('')}
          </div>
        </div>
      `;
    }).join('');
  ...
  async function populateFfmpegSidebarSection(sectionKey = '') {
    const normalizedSectionKey = String(sectionKey || '').trim();
    const container = document.getElementById(`ffmpeg-section-${normalizedSectionKey}`);
    if (!container || container.dataset.ffmpegHydrated === 'true') {
      return;
    }

    const indexData = await loadFfmpegSidebarIndex(normalizedSectionKey);
    if (!indexData) {
      return;
    }

    const kindLabel = String(indexData.kind?.displayNameArabic || indexData.kind?.displayName || normalizedSectionKey).trim();
    const kindCodicon = getFfmpegKindCodicon(normalizedSectionKey);
    const groupsHtml = renderFfmpegSidebarGroups(indexData, normalizedSectionKey);
    const fallbackItems = !groupsHtml
      ? (Array.isArray(indexData.items) ? indexData.items.map((item) => renderFfmpegSidebarItem(item, normalizedSectionKey)).join('') : '')
      : '';

    container.innerHTML = `
      <div class="nav-item" data-nav-type="ffmpeg-index" data-nav-name="${escapeAttribute(normalizedSectionKey)}" data-tooltip="يفتح فهرس ${escapeAttribute(kindLabel)} في FFmpeg داخل المرجع المعياري" tabindex="0" role="button" onclick="event.stopPropagation(); handleSidebarNavActivation(event, this); return false;">
        <span class="ui-codicon nav-icon" data-codicon="${kindCodicon}" aria-hidden="true"></span>
        <span>فهرس ${escapeHtml(kindLabel)}</span>
      </div>
      ${groupsHtml || fallbackItems}
    `;
    container.dataset.ffmpegHydrated = 'true';
    syncSidebarInteractionSemantics(container);
  }
  ...
  async function handleSidebarNavActivation(event, item) {
    ...
    const navLibrary = item.getAttribute('data-nav-library') || '';
    const navKind = item.getAttribute('data-nav-kind') || '';
    const navSlug = item.getAttribute('data-nav-slug') || '';
    if (!navType || !navName) {
      return false;
    }
    ...
    try {
      switch (navType) {
        ...
        case 'sdl3-audio-guide':
          await window.showSdl3AudioGuide();
          return true;
        case 'sdl3-example': {
        ...
        case 'sdl3':
          ...
          return true;
        case 'ffmpeg-index':
          await window.showFfmpegIndex(navName === 'ffmpeg' ? '' : navName);
          return true;
        case 'ffmpeg-entity':
          if (navLibrary === 'ffmpeg' && navKind && navSlug && typeof window.showReferenceEntity === 'function') {
            await window.showReferenceEntity(navLibrary, navKind, navSlug);
            return true;
          }
          await window.showFfmpegReference(navName);
          return true;
        default:
          return false;
      }
  ...
  async function toggleFfmpegSection(sectionKey) {
    const sectionId = 'ffmpeg-section-' + String(sectionKey || '').trim();
    const section = document.getElementById(sectionId);
    const parentSection = section?.closest('.nav-section');
    const cluster = parentSection?.closest('.nav-cluster') || document.getElementById('ffmpeg-cluster');
    ...
    if (!parentSection.classList.contains('collapsed')) {
      collapseAllSidebarSections();
      collapseAllSidebarNavGroups();
      ffmpegExpandedSections.delete(sectionKey);
      return;
    }
    ...
    try {
      await populateFfmpegSidebarSection(sectionKey);
    } catch (error) {
      console.error('Failed to populate FFmpeg sidebar section:', sectionKey, error);
    }

    collapseAllSidebarSections(parentSection);
    collapseAllSidebarNavGroups();
    parentSection.classList.remove('collapsed');
    ffmpegExpandedSections.add(sectionKey);
  }

  window.toggleFfmpegSection = toggleFfmpegSection;
