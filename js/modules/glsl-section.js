window.__ARABIC_VULKAN_GLSL_SECTION__ = (() => {
  ...
  function renderGlslSectionNavItems(sectionKey, section = {}) {
    ...
    const localizeGlslKindGroup = (kind = '', fallbackSectionTitle = sectionTitle) => {
      const value = String(kind || '').trim();
      if (!value) {
        return `عناصر ${fallbackSectionTitle}`;
      }
      const map = {
        'Built-in Sampling Function': 'دوال أخذ العينات Sampling',
        'Built-in Math Function': 'الدوال الرياضية',
        'Built-in Interpolation Function': 'دوال المزج والاستيفاء',
        'Built-in Range Function': 'دوال النطاق والحدود',
        'Built-in Vector Function': 'دوال المتجهات',
        'Built-in Matrix Function': 'دوال المصفوفات',
        'Built-in Comparison Function': 'دوال المقارنة',
        'Built-in Derivative Function': 'دوال المشتقات',
        'Built-in Atomic Function': 'الدوال الذرية',
        'Built-in Image Function': 'دوال الصور',
        'Built-in Synchronization Function': 'دوال المزامنة',
        'Built-in Check Function': 'دوال الفحص',
        'Scalar Type': 'الأنواع العددية Scalar',
        'Scalar/Return Type': 'أنواع الإرجاع والعددية',
        'Vector Type': 'أنواع المتجهات',
        'Matrix Type': 'أنواع المصفوفات',
        'Opaque Resource Type': 'أنواع الموارد المعتمة',
        'Directive': 'التوجيهات العامة',
        'Conditional Compilation Directive': 'توجيهات الشرطية والترجمة',
        'Storage Qualifier': 'مؤهلات التخزين',
        'Layout Qualifier': 'مؤهلات layout',
        'Layout Key': 'مفاتيح layout',
        'Interpolation Qualifier': 'مؤهلات الاستيفاء',
        'Memory Qualifier': 'مؤهلات الذاكرة',
        'Qualifier': 'مؤهلات عامة',
        'Boolean Literal': 'القيم المنطقية',
        'Implementation-defined Constant': 'ثوابت يحددها التنفيذ',
        'Predefined Macro': 'ماكروهات معرفة مسبقًا',
        'Built-in Constant': 'ثوابت مضمنة',
        'Built-in Input Variable': 'مدخلات مضمنة',
        'Built-in Output Variable': 'مخرجات مضمنة',
        'Built-in Interface Variable': 'متغيرات واجهة مضمنة',
        'Built-in Interface Block': 'كتل واجهة مضمنة',
        'Interface Block': 'كتل الواجهة',
        'User-defined Structure': 'البنى المعرّفة من المستخدم',
        'Control Flow': 'عناصر التحكم وتدفّق التنفيذ'
      };
      return map[value] || value;
    };

    const renderGlslNavItem = (item) => {
    ...
    (section.items || []).forEach((item) => {
      const groupTitle = localizeGlslKindGroup(item.kind, sectionTitle);
    ...
    const itemsHtml = Array.from(groups.entries()).map(([groupTitle, groupItems], index) => renderSidebarExampleGroup({
      className: 'glsl-classified-nav-group',
      dataAttributeName: 'data-glsl-kind-group',
      dataAttributeValue: `${sectionKey}-${index}`,
      isExpanded: false,
      onToggle: 'toggleConstantGroup(this)',
      titleHtml: `${escapeHtml(groupTitle)}`,
      count: groupItems.length,
      itemsHtml: groupItems.map((item) => renderGlslNavItem(item)).join('')
    })).join('');
